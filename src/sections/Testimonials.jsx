    ...
    return (
        <section className="py-20 md:py-24">
            <div className="mx-auto">
                ...
                <p className="mt-5 text-center text-lg md:text-xl tracking-tight text-white/70 max-w-xs mx-auto">Our revolutionary AI Use Cases</p>
                    ...
                    >
                        {[...testimonials, ...testimonials].map((testimonial, index) => (
                            <div key={index} className="border border-white/15 p-6 md:p-10 rounded-xl bg-[linear-gradient(to_bottom_left,rgba(6,182,212,0.3),black)] max-w-xs md:max-w-md flex-none">
                                <h2 className="text-lg md:text-2xl mb-4">{testimonial.name}</h2>
                                <p className="text-sm md:text-lg tracking-tight">{testimonial.text}</p>
                                <div className="flex items-center gap-3 mt-5">
                                    <div className="relative after:content-[''] after:absolute after:inset-0 after:bg-cyan-500 after:mix-blend-overlay before:content-[''] before:absolute before:inset-0 before:border before:border-white/30 before:z-10 before:rounded-lg">
                                        <img src={testimonial.avatarImg} alt={`Avatar for ${testimonial.name}`} height={200} width={200} className="rounded-lg grayscale border border-white/30" />
                                    </div>
                                </div>
                            </div>
                        ))}
